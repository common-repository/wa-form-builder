...
wp_register_script('wa-tinymce-functions', WP_PLUGIN_URL .'/wa-form-builder/tinyMCE/functions.js');
			...
			$output .= '<form name="add_gallery_to_post" action="">';
				$output .= '<table cellpadding="5" cellspacing="1" width="100%" bgcolor="#CCC" style="font-size:11px;">';
					$output .= '<tr>';
						...
						$output .= '</td>';
						$output .= '<td bgcolor="#F2F2F2" width="30%">';
							$output .= build_dropdown('wap_wa_form_builder').'<h3 style="float:left;">';
						$output .= '</td>';
					$output .= '</tr>';								
				$output .= '</table>';
				$output .= '<br><input type="button" name="Insert form" value="   Insert into post   " onClick="insert_wa_form();">';		
			$output .= '</form>';
	$output .= '</div>';
$output .= '</body>';
echo $output;
